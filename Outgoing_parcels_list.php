					...
					<div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable" role="document">
						<div class="modal-content">
							...
							<div class="modal-body">
								...
								<h5 class="text-primary fw-bold">To Address</h5>
								<table class="table table-striped table-bordered table-hover">
									<thead class="text-center text-dark">
										<tr>
											<th scope="col">To Location Type</th>
											<th scope="col">To Company Location</th>
											<th scope="col">To Type</th>
											<th scope="col" id="to_head">To Employee</th>
											<th scope="col">Street 1</th>
											<th scope="col">Street 2</th>
											<th scope="col">City</th>                              
											<th scope="col">State</th>                              
						...
						<div class="col-lg-12">
							<div class="card">
								<div class="text-end mt-2 mb-3 me-3">
									<a href="Add_outgoing_parcels.php">
										...
										<table id="example" class="table table-bordered table-striped table-responsive-sm">
											<thead class="text-center">
												<tr>
													...
													<th class="text-center">OP ID</th>
													<th class="text-center">Date</th>
													<th class="text-center">Outgoing Type</th>
													<th class="text-center">Item Type</th>
													<th class="text-center">No Of Items</th>
													<th class="text-center">Status</th>
												</tr>
											</thead>
											<tbody id="outgoing_parcels_list_tbody" class="cursor-pointer">
											</tbody>
										</table>
									</div>
								</div>
		...
		function get_op_list() 
		{
			$.ajax({
				...
				success: function(result) {
					var html = '';
					for(i in result) {
						html += `<tr class="info" data-opid="${ result[i].op_no }">
						<td class="text-dark text-center">${ parseInt(i) + parseInt(1) }</td>
						<td class="text-dark text-center">${ result[i].op_no }</td>
						<td class="text-dark text-center">${ result[i].date }</td>
						<td class="text-dark text-center">${ (result[0].outgoing_type == 'returnable') ? "Returnable" : "Non-Returnable" }</td>
						<td class="text-dark text-center capitalize">${ result[i].item_type }</td>
						<td class="text-dark text-center">${ result[i].no_of_items }</td>
						<td class="text-dark text-center"><span class="badge bg-info" style="width: 57px;">${ result[i].status }<span></td>
						</tr>`;
					}
					$('#outgoing_parcels_list_tbody').html(html);
					$('#example').DataTable({
						pageLength:10,
						paging:true 
					});
		...
		$(document).on('click','.info',function(){
			...
			$.ajax({
				...
				success: function(result) {
					...
					var to_addr_html = '';
					parcel_base_html += `<tr>
					<td class="text-dark text-center">${ result[0].op_no }</td>
					<td class="text-dark text-center">${ result[0].out_parcel_date }</td>
					<td class="text-dark text-center">${ (result[0].outgoing_type == 'returnable') ? "Returnable" : "Non-Returnable" }</td>
					...
					var to_name = '';
					if(result[0].to_type == 'employee') {
						to_name = result[0].to_employee_name;
						$('#to_head').text('To Employee');
					} else if(result[0].to_type == 'customer') {
						to_name = result[0].to_customer_name;
						$('#to_head').text('To Customer');
					} else if(result[0].to_type == 'vendor') {
						to_name = result[0].to_vendor_name;
						$('#to_head').text('To Vendor');
					} else if(result[0].to_type == 'govt') {
						to_name = result[0].to_govt_details;
						$('#to_head').text('To Govt');
					} else if(result[0].to_type == 'others') {
						to_name = result[0].to_other_details;
						$('#to_head').text('To Others');
					}

					to_addr_html += `<tr>
					<td class="text-dark text-center capitalize">${ result[0].to_location_type }</td>
					<td class="text-dark text-center">${ result[0].To_Plant_Description }</td>
					<td class="text-dark text-center capitalize">${ result[0].to_type }</td>
					<td class="text-dark text-center">${ to_name }</td>
					<td class="text-dark text-center">${ result[0].to_street1 }</td>
					<td class="text-dark text-center">${ result[0].to_street2 }</td>
					<td class="text-dark text-center">${ result[0].to_city }</td>
					<td class="text-dark text-center">${ result[0].to_state }</td>
					<td class="text-dark text-center">${ result[0].to_country }</td>
					<td class="text-dark text-center">${ result[0].to_pincode }</td>
					<td class="text-dark text-center">${ result[0].to_phone_no }</td>
					</tr>`;
